...
class GNNLayer(nn.Module):
    ...
    References:
        - X. Bresson and T. Laurent. An experimental study of neural networks for variable
    ...
    def __init__(self, hidden_dim, aggregation="sum", norm="batch", learn_norm=True,
                 track_norm=False, gated=True):
        """
        Args:
...
class GNNEncoder(nn.Module):
    ...
    def dense_forward(self, x, graph, timesteps, edge_index=None):
        ...
        for layer, time_layer, out_layer in zip(self.layers, self.time_embed_layers,
                                                self.per_layer_out):
            x_in, e_in = x, e
