...
class MISModel(COMetaModel):
    def __init__(self,
        ...
        if self.args.training_split_label_dir is not None:
            data_label_dir = os.path.join(self.args.storage_path,
    ...
    def test_step(self, batch, batch_idx, draw=False, split='test'):
        ...
        solved_solutions = [mis_decode_np(predict_labels, adj_mat) for predict_labels in
        ...
        self.log(f"{split}/solved_cost", best_solved_cost, prog_bar=True, on_epoch=True,
                 sync_dist=True)
        return metrics
