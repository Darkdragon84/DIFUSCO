...
def parse_path(results_json_path: str, outputs_path: str) -> re.Match:
    ...
    match = re.match(
        r"\/(?P<solver>\S+?)\/(?P<dataset>\S+)\/test(?:\/(?P<config>\S+))?\/results\.json",
        results_json_path)
    if not match:
    ...
    for result_file in args.experiment_output_folder.rglob("results.json"):
        ...
        for graph, metrics in parse_output_json(result_file, solver):
            if graph is not None and metrics is not None:
                rows.append((solver, config, graph + "_weighted" if weighted else graph) + tuple(
                    metrics.get(metric_name) for metric_name in metric_names))
...
if __name__ == "__main__":
    ...
    parser.add_argument("experiment_output_folder", type=pathlib.Path, action="store",
                        help="Folder in which the output by the experiments are stored.")
