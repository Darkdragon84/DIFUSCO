...
class RealWorldGraphGenerator(DataGenerator):
        ...
        if self.create_if_needed(name):
            ...
            for graph_idx, file in enumerate(file_list):
                ...
                with GraphDataset(file_url) as dsdir:
                    df = pd.read_csv(dsdir / file.replace(".tar.gz", "") / "conflict_graph.txt",
                                     sep=" ", header=None, skiprows=1, names=["source", "target"])

                    G = nx.from_pandas_edgelist(df)
                    weight_mapping = {}
                    with open(dsdir / file.replace(".tar.gz", "") / "node_weights.txt",
                              "r") as a_file:
                        for line in a_file:
                    ...
                    nx.write_gpickle(G, str(self.get_dataset_directory(
                        name) / f"{name}-{graph_idx}.gpickle"))
                    logger.info(
                        f"Saved {name}-{graph_idx}. nodes={G.number_of_nodes()}, "
        ...
        if self.create_if_needed(name):
            with GraphDataset(url) as dsdir:
                ...
                with open(dsdir / fullname / f"{fullname}_A.txt") as file:
                    for line in file:
                        _line = list(map(lambda s: int(s),
                                         line.strip().split(", ")))  # space is there on purpose!
                        u = _line[0]
                        v = _line[1]
                        if node_to_graph_dict[u] != node_to_graph_dict[v]:
                            logger.info(
                                f"Error u={u}, v={v}, graph(u) = {node_to_graph_dict[u]}, "
                                f"graph(v) = {node_to_graph_dict[v]}")
    ...
    def generate(self, gen_labels=False, weighted=False):
        ...
        self.handle_reddit("reddit-b", "REDDIT-BINARY",
                           "https://ls11-www.cs.tu-dortmund.de/people/morris/graphkerneldatasets"
                           "/REDDIT-BINARY.zip",
                           gen_labels)
        self.handle_reddit("reddit-5k", "REDDIT-MULTI-5K",
                           "https://ls11-www.cs.tu-dortmund.de/people/morris/graphkerneldatasets"
                           "/REDDIT-MULTI-5K.zip",
                           gen_labels)
        self.handle_reddit("reddit-12k", "REDDIT-MULTI-12K",
                           "https://ls11-www.cs.tu-dortmund.de/people/morris/graphkerneldatasets"
                           "/REDDIT-MULTI-12K.zip",
                           gen_labels)

        self.handle_amazon("amazon-mt",
                           ["MT-D-01.tar.gz", "MT-D-200.tar.gz", "MT-D-FN.tar.gz", "MT-W-01.tar.gz",
                            "MT-W-200.tar.gz", "MT-W-FN.tar.gz"], gen_labels)
        self.handle_amazon("amazon-mr",
                           ["MR-D-01.tar.gz", "MR-D-03.tar.gz", "MR-D-05.tar.gz", "MR-D-FN.tar.gz",
                            "MR-W-FN.tar.gz"], gen_labels)
        self.handle_amazon("amazon-mw",
                           ["MW-D-01.tar.gz", "MW-D-20.tar.gz", "MW-D-40.tar.gz", "MW-D-FN.tar.gz",
                            "MW-W-01.tar.gz", "MW-W-05.tar.gz", "MW-W-10.tar.gz", "MW-W-FN.tar.gz"],
                           gen_labels)
        self.handle_amazon("amazon-cw", ["CW-T-C-1.tar.gz", "CW-T-C-2.tar.gz", "CW-T-D-4.tar.gz",
                                         "CW-T-D-6.tar.gz"], gen_labels)
        self.handle_amazon("amazon-cr", ["CR-T-C-1.tar.gz", "CR-T-C-2.tar.gz", "CR-T-D-4.tar.gz",
                                         "CR-T-D-6.tar.gz", "CR-T-D-7.tar.gz"], gen_labels)
        ...
        if self.create_if_needed("as-caida"):
            with GraphDataset("https://snap.stanford.edu/data/as-caida.tar.gz") as dsdir:
                for idx, graph_path in enumerate(dsdir.rglob("*.txt")):
                    ...
                    df = pd.read_csv(graph_path, skiprows=8, sep="\t", header=None,
                                     names=["source", "target", "label"])
                    G = clean_nx_graph(nx.from_pandas_edgelist(df))
                    G = self.maybe_label_graph(gen_labels, G, f"as-caida-{idx}")
                    nx.write_gpickle(G, str(self.get_dataset_directory(
                        "as-caida") / f"as-caida-{idx}.gpickle"))
                    logger.info(
                        f"Saved as-caida-{idx}. nodes={G.number_of_nodes()}, "
                        f"edges={G.number_of_edges()}")

        if self.create_if_needed("citeseer"):
            with GraphDataset("https://nrvis.com/download/data/labeled/citeseer.zip") as dsdir:
                df = pd.read_csv(dsdir / "citeseer.edges", header=None,
                                 names=["source", "target", "label"])
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "citeseer")
                nx.write_gpickle(G,
                                 str(self.get_dataset_directory("citeseer") / "citeseer.gpickle"))
                logger.info(
                    f"Saved citeseer. nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")

        if self.create_if_needed("cora"):
            with GraphDataset("https://nrvis.com/download/data/labeled/cora.zip") as dsdir:
                G = clean_nx_graph(nx.from_pandas_edgelist(
                    pd.read_csv(dsdir / "cora.edges", header=None,
                                names=["target", "source", "label"])))
        ...
        if self.create_if_needed("wiki-Vote"):
            with GraphDataset("https://snap.stanford.edu/data/wiki-Vote.txt.gz") as dsdir:
                df = pd.read_csv(dsdir / "wiki-Vote.txt", header=None, names=["source", "target"],
                                 sep="\t", skiprows=4)
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "wiki-Vote")
                nx.write_gpickle(G,
                                 str(self.get_dataset_directory("wiki-Vote") / "wiki-Vote.gpickle"))
                logger.info(
                    f"Saved wiki-Vote. nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")

        if self.create_if_needed("wiki-RfA"):
            with GraphDataset(
                    "https://suitesparse-collection-website.herokuapp.com/MM/SNAP/wiki-RfA.tar.gz"
                    "") as dsdir:
                ...
                nx.write_gpickle(G,
                                 str(self.get_dataset_directory("wiki-RfA") / "wiki-RfA.gpickle"))
                logger.info(
                    f"Saved wiki-RfA. nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")

        if self.create_if_needed("bitcoin-otc"):
            with GraphDataset("https://snap.stanford.edu/data/soc-sign-bitcoinotc.csv.gz") as dsdir:
                df = pd.read_csv(dsdir / "soc-sign-bitcoinotc.csv", header=None,
                                 names=["source", "target", "rating", "time"])
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "bitcoin-otc")
                nx.write_gpickle(G, str(self.get_dataset_directory(
                    "bitcoin-otc") / "bitcoin-otc.gpickle"))
                logger.info(
                    f"Saved bitcoin-otc. nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")

        if self.create_if_needed("bitcoin-alpha"):
            with GraphDataset(
                    "https://snap.stanford.edu/data/soc-sign-bitcoinalpha.csv.gz") as dsdir:
                df = pd.read_csv(dsdir / "soc-sign-bitcoinalpha.csv", header=None,
                                 names=["source", "target", "rating", "time"])
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "bitcoin-alpha")
                nx.write_gpickle(G, str(self.get_dataset_directory(
                    "bitcoin-alpha") / "bitcoin-alpha.gpickle"))
                logger.info(
                    f"Saved bitcoin-alpha. nodes={G.number_of_nodes()}, edges="
                    f"{G.number_of_edges()}")

        if self.create_if_needed("roadnet-pennsylvania"):
            with GraphDataset("https://snap.stanford.edu/data/roadNet-PA.txt.gz") as dsdir:
                df = pd.read_csv(dsdir / "roadNet-PA.txt", sep="\t", header=None, skiprows=4,
                                 names=["source", "target"])
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "roadnet-pennsylvania")
                nx.write_gpickle(G, str(self.get_dataset_directory(
                    "roadnet-pennsylvania") / "roadnet-pennsylvania.gpickle"))
                logger.info(
                    f"Saved roadnet-pennsylvania. nodes={G.number_of_nodes()}, "
                    f"edges={G.number_of_edges()}")

        if self.create_if_needed("web-google"):
            with GraphDataset("https://snap.stanford.edu/data/web-Google.txt.gz") as dsdir:
                df = pd.read_csv(dsdir / "web-Google.txt", sep="\t", header=None, skiprows=4,
                                 names=["source", "target"])
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "web-google")
                nx.write_gpickle(G, str(self.get_dataset_directory(
                    "web-google") / "web-google.gpickle"))
                logger.info(
                    f"Saved web-google. nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")

        if self.create_if_needed("ego-gplus"):
            with GraphDataset("https://snap.stanford.edu/data/gplus_combined.txt.gz") as dsdir:
                df = pd.read_csv(dsdir / "gplus_combined.txt", sep=" ", header=None,
                                 names=["source", "target"])
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "ego-gplus")
                nx.write_gpickle(G,
                                 str(self.get_dataset_directory("ego-gplus") / "ego-gplus.gpickle"))
                logger.info(
                    f"Saved ego-gplus. nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")

        if self.create_if_needed("ego-facebook"):
            with GraphDataset("https://snap.stanford.edu/data/facebook_combined.txt.gz") as dsdir:
                df = pd.read_csv(dsdir / "facebook_combined.txt", sep=" ", header=None,
                                 names=["source", "target"])
                G = clean_nx_graph(nx.from_pandas_edgelist(df))
                G = self.maybe_label_graph(gen_labels, G, "ego-facebook")
                nx.write_gpickle(G, str(self.get_dataset_directory(
                    "ego-facebook") / "ego-facebook.gpickle"))
                logger.info(
                    f"Saved ego-facebook. nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")
        ...
        if self.create_if_needed("ppi"):
            if not is_module_available("dgl"):
                ...
                for idx, G in enumerate(nx_graphs):
                    ...
                    logger.info(
                        f"Saved ppi-{idx}. nodes={G.number_of_nodes()}, edges="
                        f"{G.number_of_edges()}")
        ...
        if self.create_if_needed("roadnet-berlin"):
            if not is_module_available("osmnx") or not is_module_available("dgl"):
                ...
                G = dgl.from_networkx(
                    G)  # need to do this, otherwise nx graph is dirty from osmnx stuff
                G = clean_nx_graph(dgl.to_networkx(G))
                G = self.maybe_label_graph(gen_labels, G, "roadnet-berlin")
                nx.write_gpickle(G, str(self.get_dataset_directory(
                    "roadnet-berlin") / "roadnet-berlin.gpickle"))
                logger.info(
                    f"Saved roadnet-berlin. nodes={G.number_of_nodes()}, "
                    f"edges={G.number_of_edges()}")
        ...
        if self.create_if_needed("kexu-vc-benchmark"):
            ...
            for variant in ["30-15", "35-17", "40-19", "45-21", "50-23", "53-24", "56-25", "59-26"]:
                for instance in range(1, 6):
                    with GraphDataset(
                            f"https://raw.githubusercontent.com/unsat/npbench/master/instances"
                            f"/vertex_cover/benchmarks/frb{variant}-mis/frb{variant}-"
                            f"{instance}.mis",
                            unpack=False) as dsdir:
                        df = pd.read_csv(dsdir / f"frb{variant}-{instance}.mis", sep=" ",
                                         header=None, skiprows=1, names=["p", "source", "target"])
                        G = clean_nx_graph(nx.from_pandas_edgelist(df))
                        G = self.maybe_label_graph(gen_labels, G, f"kexu-vc-benchmark-{idx}")
                        nx.write_gpickle(G, str(self.get_dataset_directory(
                            "kexu-vc-benchmark") / f"kexu-vc-benchmark-{idx}.gpickle"))
                        logger.info(
                            f"Saved kexu-vc-benchmark-{idx}. nodes={G.number_of_nodes()}, "
                            f"edges={G.number_of_edges()}")
                        idx += 1

        if self.create_if_needed("dimacs"):
            with GraphDataset(
                    f"http://lcs.ios.ac.cn/~caisw/Resource/DIMACS complementary graphs.tar.gz",
                    unpack=True) as dsdir:
                for idx, graph_path in enumerate(dsdir.rglob("*.mis")):
                    df = pd.read_csv(graph_path, sep=" ", header=None, skiprows=1,
                                     names=["p", "source", "target"])
                    G = clean_nx_graph(nx.from_pandas_edgelist(df))
                    G = self.maybe_label_graph(gen_labels, G, f"dimacs-{idx}")
                    nx.write_gpickle(G, str(self.get_dataset_directory(
                        "dimacs") / f"dimacs-{idx}.gpickle"))
                    logger.info(
                        f"Saved dimacs-{idx}. nodes={G.number_of_nodes()}, "
                        f"edges={G.number_of_edges()}")
