...
def run_command_with_live_output(command, shell=False, capture_output=False):
    if not capture_output:
        result = subprocess.run(command, shell=shell, stdout=sys.stdout, stderr=sys.stderr,
                                text=True)
        return result, None
    else:
...
def setup_conda_env_from_file(environment_file, force=False):
    ...
    run_command_with_live_output(
        ['conda', 'env', 'create', '-f', environment_file, '--force' if force else ''])
...
def launch_python_script_in_conda_env(environment_name, environment_file, script, args=[]):
    ...
    # run_command_with_live_output(["conda" , "run" , "--no-capture-output", "-n",
    # environment_name, "python", script] + list(map(lambda x: str(x), args)))
    run_command_with_live_output(["conda", "run", "-n", environment_name, "python", script] + list(
